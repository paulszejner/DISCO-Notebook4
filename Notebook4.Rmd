...
In this exercise, you will model the oxygen isotopes in cellulose ```d18O_cellulose```. However, this exercise is intended for you to evaluate the sensitivity of the different variables. Namely, how cellulose 18O will vary if we vary the relative humidity ```relH```,  the source water ```water```, the stomatal conductance```gs``` and the percentage of exchange during the cellulose sinthesys```F_0```


## PART 1: Variables

* First create a vector [```relH``` ] of relative humidity ranging from 10% to 90%.   
* Second, create a vector [```water```] of Source water ranging from -20 permill to 5 permill.  
* Third, create a 2 column object [```RH_SOURCE```] combining  ```relH```  and ```water``` that correspond to all possible combination of relative humidity and source water 18O. 
...
RH_SOURCE <- merge(relH, water) # here we merge the two variables tofacilitate the calculations on each row of this object.

#Subset source water and RH combivations
Source_18O <- RH_SOURCE[,2]
Relative_humidity <- RH_SOURCE[,1]
```

Plot RH and Source water in order to visualize all the possible RH-Source water combinations. Example: for RH= 20% there are 16 Source water values ranging from
-25 to 5 permill.
```{r}
plot(Relative_humidity, Source_18O)
```

These RH-Source water combinations will be used as input for tree ring cellulose model. In addition, other variables such as stomatal conductances ```gs``` and   percentage of  isotope exchange during cellulose synthesis ```F_0``` will also be varied.

## PART 2: Modeling the Oxygen Isotopes in the tree ring cellulose
The leaf water enrichment and cellulose 18O will be predicted  using an extensive range of variability of the four critical factors in the model: source water, RH, gs and F_0.

```{r}
# Constants -----------------
br <-1 # boundary layer resistance, considered reasonable value between 0.02 to 3    ak <- 1.0285	# alpha_kappa, Kinetic Fractionation 
...
Ta <- 25 # In this exercise, we are using a constant temperature, air temp. (°C)  Assumption that Tleaf= Tair
```

First, we estimate the ambiant vapor pressure as function of Source water and RH:
``` {r}
water_vapor <-Source_18O +(-1*(1.137*(1000000/((Ta+273.15)^2))-0.4156*((1000/(Ta+273.15))-2.0667))) 

# "es" is the saturation vapor pressure (kPa) with air temp in this case at 25
...
# ea_v is the ambient vapor pressure (kPa)
RH_v <- RH_SOURCE[,1]
ea_v =(RH_v/100)*es
```

Plot and describe the relationship between the Relative humidity and the ambient vapor pressure (kPa):
``` {r}
...
``` {r}
plot(RH_SOURCE[,1],E_v_gs[[1]],type="l", xlim = c(10,90), range(E_v_gs), ylab="leaf transpiration, mol m-2 s-1", xlab="rel Humidity")
lines(RH_SOURCE[,1],E_v_gs[[2]], lwd=3)
lines(RH_SOURCE[,1],E_v_gs[[3]], lwd=6)
legend("topright", legend = c("gs=0.01","gs=0.25","gs=0.5" ), lwd = c(1,3,6))
```
We have three different stomatal conductances ```gs```, and in this graphic, we can see different slopes between relative humidity and the estimated leaf transpiration flux on other gs. 

Describe this relationship between the relative humidity and leaf transpiration (mol m-2 s-1) under three different stomatal conductance values: 
-  
-  
-  
...
legend("topright", legend = c("gs=0.01","gs=0.25","gs=0.5" ), lwd = c(1,3,6))
```
Describe this plot Recall the  range of source water  and  now we can observe the effects depending on the amount of water at the leaf surface:  
-  
-  
-  
-  
...
legend("topright", legend = c("gs=0.01","gs=0.25","gs=0.5" ), lwd = c(1,3,6))
```
Describe this plot, remember the range of source water and observe the effects on the leaf water enrichment depending on the environmental relative humidity:  
-  
-  
-  
-  
...
In this final step, you will estimate the different sensitivities the ```d18O_cellulose``` is exposed to under different environments and different factors, and you will need to conclude what factors analyzed in this exercise are the most sensitive and why. 
...
In this final step, we will  plot all in figure with all the values  estimated in this exercise a three-by-three panel using heat maps of the  RH, Source vectors.
