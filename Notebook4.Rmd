...
for (sto in 1:3){E[[sto]]=((ei-ea)/press_bar)*g[sto] }
```
...
ws <- list()
for(sto in 1:3){ws[[sto]] =((gs[[sto]]*wi)-E[[sto]]*(1-wi/2))/(gs[[sto]] - E[[sto]]/2)}

# Calculate vapor pressure at leaf surface (es)
es<- list()
for(sto in 1:3){es[[sto]]=ws[[sto]]*press_bar}
...
for(sto in 1:3){
  Rleaf[[sto]] <- alpha_star*((ak*Rsource*((ei-es[[sto]])/ei))+
                              (akb*Rsource*((es[[sto]]-ea)/ei))+
                              (Ratm*(ea/ei)))
                              }
```
...
for(sto in 1:3){
  d18Oleaf[[sto]]=((Rleaf[[sto]]/rsmow)-1)*1000 
}
```
...
for(sto in 1:3){
  d18Oc_F_0 <- list()
  for (pe in 1:3){ # variable percentage of Exchange during Cellulose Synthesis
    d18Oc_F_0[[pe]] <- F_0[[pe]]*(Source_18O + epsilon_o) + (1 - F_0[[pe]]) * (d18Oleaf[[sto]]+ epsilon_o) 
  }
...
Check the structure of the d18O_cellulose object. We generated a list of 3 stomatal conductance variations and for each stomatal conductance value, three different Percentage of exchange during cellulose sythesis.
``` {r}
str(d18O_cellulose)
```
