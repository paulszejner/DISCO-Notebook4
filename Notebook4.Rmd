...
In this exercise, you will model the oxygen isotopes in cellulose. However, this exercise is intended for you to evaluate the sensitivity of the different variables. For example, what happens to the cellulose oxygen isotopes if we vary the relative humidity ```relH```,  the source water ```water```, the stomatal conductance```gs``` and the percentage of exchange during the cellulose sinthesys```F_0```
...
to this point we  have  all the components to estimate what will be the magnitud of the isotopic enrichment at the leaf level, because we already have calculations for internal leaf vapor pressure (kPa)```ei```,vapor pressure at leaf surface (kPa) ```esl_v_gs```, ambient vapor pressure (kPa)```ea_v```  under different Stomatal conductances ```gs```, and different relative humidities```relH``` and, source waters ```water```.
...
points(esl_v_gs[[3]][1:17],Rleaf_v_gs[[3]][1:17], lwd=3)
points(esl_v_gs[[5]][1:17],Rleaf_v_gs[[5]][1:17], lwd=6)
...
points(esl_v_gs[[3]][1:17],d18OL_gs[[3]][1:17], lwd=3)
points(esl_v_gs[[5]][1:17],d18OL_gs[[5]][1:17], lwd=6)
...
lines(RH_v[1:17], d18OL_gs[[3]][1:17], lwd=3)
lines(RH_v[1:17], d18OL_gs[[5]][1:17], lwd=6)
